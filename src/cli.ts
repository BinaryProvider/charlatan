...
export class CLI {
  ...
  public static error(text: string, leadingSpace?: boolean, trailingSpace?: boolean): void {
    if (leadingSpace) console.log('');
    console.log(format(`^R^+${text}`));
    if (trailingSpace) console.log('');
  }

  public static success(text: string, leadingSpace?: boolean, trailingSpace?: boolean): void {
    if (leadingSpace) console.log('');
    console.log(format(`^G^+${text}`));
    if (trailingSpace) console.log('');
  }

  public static note(text: string, leadingSpace?: boolean, trailingSpace?: boolean): void {
    if (leadingSpace) console.log('');
    console.log(format(`^b${text}`));
    if (trailingSpace) console.log('');
  }

  public static info(text: string, leadingSpace?: boolean, trailingSpace?: boolean): void {
    if (leadingSpace) console.log('');
    console.log(format(`^w${text}`));
    if (trailingSpace) console.log('');
  }

  public static text(text: string, leadingSpace?: boolean, trailingSpace?: boolean): void {
    if (leadingSpace) console.log('');
  ...
  public static help(): void {
    const logo = figlet.textSync('       charlatan       ', { font: 'Rounded' });
    ...
    table.push(
      ...
      [this.argFormat('--definitions'), this.descFormat('Custom schema definition')],
      [this.argFormat('--definitionDir'), this.descFormat('Directory with schema definitions')],
      ...
      [this.argFormat('--options.file.name.find'), this.descFormat('Filename find (regex)')],
      [this.argFormat('--options.file.name.replace'), this.descFormat('Filename replace (string)')],
      [this.argFormat('--options.model'), this.descFormat('Model options')],
      [this.argFormat('--options.model.name'), this.descFormat('Model name options')],
      [this.argFormat('--options.model.name.find'), this.descFormat('Model name find (regex)')],
      [this.argFormat('--options.model.name.replace'), this.descFormat('Model name replace (string)')],
      [this.argFormat('--options.endpoint'), this.descFormat('Endpoint options')],
      [this.argFormat('--options.endpoint.name'), this.descFormat('Endpoint name options')],
      [this.argFormat('--options.endpoint.name.find'), this.descFormat('Endpoint name find (regex)')],
      [this.argFormat('--options.endpoint.name.replace'), this.descFormat('Endpoint name replace (string)')],
    );
  ...
  private static argFormat(input: string): string {
    const parts = input.split('.').map((part, index) => `${this.ARG_COLORS[index]}${part}`);
