...
export class SchemaParser {
  ...
  public static createSchemaDefinitions(
    ...
    customSchemaDefinitions: SchemaDefinition[]
  ): void {
    ...
    endpoints?.forEach(async (endpoint) => {
      const customDefinition = this.parseCustomSchemaDefinition(
        endpoint,
        customSchemaDefinitions
      );
      const defaultSchema = this.createSchemaDefinition(endpoint);
      const customSchema = this.applyCustomSchemaDefinition(
        endpoint,
        customDefinition
      );
  ...
  public static async parseCustomDefinitions(
    files: string[]
  ): Promise<SchemaDefinition[]> {
    return await Promise.all(files.map((file) => import(file)));
  }

  private static setInitialSchemaDefinitionIndexes(
    customDefinitions: SchemaDefinition[]
  ): void {
  ...
  private static applyCustomSchemaDefinition(
    ...
    customDefinition: SchemaDefinition
  ): any {
      ...
      schema?.routes.forEach((schema) => {
        ...
        if (schema.response) {
          route.methods.forEach((method) => {
            const response = schema.response.find(
              (response) =>
                response.verb &&
                response.verb.toLowerCase() === method.verb.toLowerCase()
            );
  ...
  private static parseCustomSchemaDefinition(
    endpoint: EndpointData,
  ...
  private static generateGeneric(
    property: unknown,
  ...
  private static generateObject(object: unknown): unknown {
    ...
    properties.forEach((key) => {
      ...
      const createPropertyDefinition =
